...
const AdminDashboard = () => {
  const {
    ...
    updateEnrollmentStatus,
    updateAccessType,
    extendTrial, // 👈 make sure this exists in your hook
  } = useAdminData();
  ...
  const filterLearners = (list: LearnerRow[]) =>
    list.filter((l) => {
      const matchSearch =
        ...
        l.profile.email.toLowerCase().includes(search.toLowerCase());
      const matchStatus =
        statusFilter === "all" || l.enrollment?.status === statusFilter;
      const matchTrack =
        trackFilter === "all" ||
        l.enrollment?.learning_track === trackFilter;
      return matchSearch && matchStatus && matchTrack;
    });

  const filteredPaid = useMemo(
    () => filterLearners(paidLearners),
    [paidLearners, search, statusFilter, trackFilter]
  );

  const filteredFree = useMemo(
    () => filterLearners(freeLearners),
    [freeLearners, search, statusFilter, trackFilter]
  );
  ...
  const conversionRate =
    stats.paid + stats.free > 0
      ? ((stats.paid / (stats.paid + stats.free)) * 100).toFixed(1)
      : "0";
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm opacity-80">
                  Platform analytics & learner oversight
                </p>
              </div>
            </div>
          ...
          <Tabs defaultValue="paid">
            <TabsList className="mb-6">
              ...
              <TabsTrigger value="free">
                Free Trial ({filteredFree.length})
              </TabsTrigger>
            </TabsList>
